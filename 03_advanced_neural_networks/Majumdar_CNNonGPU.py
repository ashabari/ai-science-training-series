...
logging.basicConfig(filename='/content/drive/My Drive/Majumdar_CNN/original_training_log.log', level=logging.INFO, 
...
data_path = '/content/drive/My Drive/Majumdar_CNN/dataset'
...
training_data = torchvision.datasets.CIFAR10(
    ...
    transform=v2.Compose([
        ...
        v2.ToDtype(torch.float32, scale=True),  # Convert to float32 tensor
...
class ConvNextBlock(nn.Module):
    def __init__(self, in_channels, shape):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, 
                                     ...
                                     groups=in_channels,
                                     kernel_size=7,
                                     padding=3)  # Set padding to maintain spatial dimensions
        self.norm = nn.LayerNorm([in_channels, *shape])
...
class Classifier(nn.Module):
    def __init__(self, n_initial_filters, n_stages, blocks_per_stage):
        ...
        current_shape = [32, 32] # Shape is now maintained
...
model = Classifier(128, 4, 3)  # Changed initial filters to 128 and blocks per stage to 3
model.to(dev)
...
optimizer = torch.optim.AdamW(model.parameters(), lr=0.005, weight_decay=0.01)  # Change learning rate to 0.005
...
# Save model and logs
torch.save(model.state_dict(), '/content/drive/My Drive/Majumdar_CNN/model.pth')
files.download('/content/drive/My Drive/Majumdar_CNN/model.pth')
files.download('/content/drive/My Drive/Majumdar_CNN/training_log.log')
